...
def load_images_batch(dir_path, start_idx, batch_size):
    """分批加载图片"""
    ...
    all_files = sorted(Path(dir_path).glob("*/*/*.png"))
...
all_files = sorted(Path(pred_dir).glob("*/*/*.png"))
...
for i in range(0, num_images, batch_size):
    ...
    pred_batch = load_images_batch(pred_dir, i, batch_size)
    gt_batch = load_images_batch(gt_dir, i, batch_size)
