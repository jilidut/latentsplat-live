...
class DecoderSplattingCUDA(Decoder[DecoderSplattingCUDACfg]):
    ...
    def render_to_decoder_output(
        ...
        uncertainty = None
        print(f"[DEBUG] render_output.feature is None: {render_output.feature is None}")
        if render_output.feature is not None:
            features = rearrange(render_output.feature, "(b v) c h w -> b v c h w", b=b, v=v)
            B, V, C, H, W = features.shape
            print(f"[DEBUG] feature shape: {features.shape}, C={C}")
            print(f"[DEBUG] self.variational={self.variational}")
        features = None
        if render_output.feature is not None:
            features = rearrange(render_output.feature, "(b v) c h w -> b v c h w", b=b, v=v)
            C = features.shape[2]
            ...
            if C == 65:
                gaussian_latent = features.contiguous()
                semantic_latent = features[:, :, 32:64].contiguous()
                uncertainty = F.softplus(features[:, :, 64:65]) + 1e-6
                uncertainty = uncertainty.clamp(max=10.0).contiguous()
                ...
                feature_posterior = DiagonalGaussianDistribution(
                    torch.cat([mean_flat, logvar_flat], dim=-1),
                    dim=-1
                )
            else:  
                mean_flat   = rearrange(features, "b v c h w -> b (v h w) c")
