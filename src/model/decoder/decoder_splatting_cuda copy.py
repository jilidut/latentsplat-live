...
class DecoderSplattingCUDA(Decoder[DecoderSplattingCUDACfg]):
    ...
    #     color_sh = None
    #     if gaussians.color_harmonics is not None:
    #         color_sh = repeat(gaussians.color_harmonics, "b g c d_sh -> (b v) g c d_sh", v=v).to(device)
    #     feature_sh = None
    #     if return_features and hasattr(gaussians, 'features') and gaussians.features is not None:
    #         feature_sh = repeat(gaussians.features, "b g c d_sh -> (b v) g c d_sh", v=v).to(device)
    #     if feature_sh is not None:
    #         feature_sh = feature_sh.to(device)

    #     H, W = image_shape[-2:]   # 安全写法
