...
class ModelWrapper(LightningModule):
    ...
    def __init__(
        ...
        print("[DEBUG] ModelWrapper.__init__() called")
        super().__init__()
        ...
        # 获取配置中的高斯损失配置
        gaussian_loss_cfg = cfg.loss.gaussian  # 从 cfg 中获取正确的高斯损失配置
        print(f"[DEBUG] ModelWrapper: gaussian_loss_cfg = {gaussian_loss_cfg}")

        # 初始化高斯损失
        if gaussian_loss_cfg is not None:
            self.gaussian_losses = get_loss_group("gaussian", gaussian_loss_cfg)
            print(f"[DEBUG] gaussian_losses initialized as: {self.gaussian_losses}")
            print(f"[DEBUG] ModelWrapper: gaussian_losses = {self.gaussian_losses}")
        else:
            print("[DEBUG] cfg.loss.gaussian is None")
    ...
    def get_active_loss_groups(self):
        return {
            ...
            "gaussian": self.gaussian_losses.is_active(self.step_tracker.get_step()) if self.gaussian_losses is not None else False,
    ...
    def training_step(self, batch, batch_idx):
        ...
        if self.gaussian_losses is not None:
            print("gaussian_losses.is_active():", self.gaussian_losses.is_active(self.step_tracker.get_step()))
            ...
            print("gaussian_loss_dict keys:", gaussian_loss_dict.keys())
        else:
            print("gaussian_losses is None, skipping forward_generator")
        ...
        print("loss_dict keys:", loss_dict.keys())
        # self.log("loss/l1",   loss_dict["l1"],   prog_bar=False)
        # self.log("loss/ssim", loss_dict["ssim"], prog_bar=False)
