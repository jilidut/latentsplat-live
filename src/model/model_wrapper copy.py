    ...
    def training_step(self, batch, batch_idx: int):
        ...
        for loss_group, pred, gt in zip(
            ...
            if loss_group is None:
                continue
            print(f"[DEBUG] Loss group: {loss_group.name}")
            if pred.image is not None:
                print(f"[DEBUG] pred.image.requires_grad: {pred.image.requires_grad}")
            if gt.image is not None:
                print(f"[DEBUG] gt.image.requires_grad: {gt.image.requires_grad}")
        ...
        if self.discriminator is not None:
            ...
            for loss_group, pred, gt in zip(
                ...
                if loss_group.is_discriminator_loss_active(self.step_tracker.get_step()):
                    b, v = pred.image.shape[:2]
                    print(f"[DEBUG] Discriminator input pred.image.requires_grad: {pred.image.requires_grad}")
                    logits_fake = self.discriminator(
