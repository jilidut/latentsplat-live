...
class ModelWrapper(LightningModule):
    ...
    def __init__(
        ...
        # # 新增 -------------- G-S-U 框架损失 --------------
        # # 从已传进来的 loss_cfg 里读 gsu 节点
        # loss_root_cfg = target_render_image_loss_cfg
        # self.gsu_cfg = dataclasses.asdict(loss_root_cfg).get("gsu", None)

        # if self.gsu_cfg:
        #     from src.loss.geom_sem_contrast import GeomSemContrastLoss
        #     from src.loss.unc_sem_couple   import UncSemCoupleLoss
        #     self.geom_sem_loss = GeomSemContrastLoss(self.gsu_cfg["geom_sem_contrast"])
        #     self.unc_sem_loss  = UncSemCoupleLoss(self.gsu_cfg["unc_sem_couple"])
        # else:
        ...
        # 新增 G-S-U 框架损失
        gsu_cfg = getattr(target_render_image_loss_cfg, "gsu", None)
        self.gsu_cfg = gsu_cfg if gsu_cfg else None

        if self.gsu_cfg:
            from src.loss.geom_sem_contrast import GeomSemContrastLoss
            from src.loss.unc_sem_couple import UncSemCoupleLoss
            self.geom_sem_loss = GeomSemContrastLoss(self.gsu_cfg.get("geom_sem_contrast", {}))
            self.unc_sem_loss = UncSemCoupleLoss(self.gsu_cfg.get("unc_sem_couple", {}))
        else:
            self.geom_sem_loss = None
            self.unc_sem_loss = None
        # 新增 ---------------------------------------------
    ...
    def training_step(self, batch, batch_idx):
        ...
        generator_loss = 0
        for loss_group, pred, gt in zip( # 新增, self.target_render_image_gsu_losses
            (self.gaussian_losses, self.context_losses, self.target_autoencoder_losses, self.target_render_image_losses, self.target_render_latent_losses, self.target_combined_losses, self.target_render_image_gsu_losses),
        ...
        # # 新增==========  G-S-U 框架损失 ==========
        # if self.geom_sem_loss is not None and self.unc_sem_loss is not None:
        #     # 1. 提取张量（名字与 decoder 返回对齐）
        #     z_geom   = output["gaussian_latent"][..., :32].mean(dim=(2,3))   # [B, D]
        #     z_sem    = output["semantic_latent"][..., :32].mean(dim=(2,3))   # [B, D]
        #     sigma    = output["gaussian_latent"][..., 32:33].mean(dim=(2,3)) # [B, 1]
        #     sem_logits = output["semantic_logits"]                           # [B, C, H, W]
        #     sem_target = batch["target"].get("semantic")                     # [B, H, W]

        #     # 2. 计算 loss
        #     gs_loss = self.geom_sem_loss(z_geom, z_sem)
        #     us_loss = self.unc_sem_loss(sem_logits, sigma.unsqueeze(-1), sem_target)

        #     # 3. 累加到总 loss
        #     gsu_weight_gs = self.gsu_cfg.geom_sem_contrast.weight
        #     gsu_weight_us = self.gsu_cfg.unc_sem_couple.weight
        #     generator_loss = generator_loss + gsu_weight_gs * gs_loss + gsu_weight_us * us_loss

        #     # 4. 日志（TensorBoard 自动显示）
        #     self.log("loss/geom_sem_contrast", gs_loss)
        #     self.log("loss/unc_sem_couple",   us_loss)
        # # 新增=========================================

        print(">>> GS/US 状态检查: step =", self.global_step,
      "geom_sem_loss实例=", self.geom_sem_loss is not None,
      "unc_sem_loss实例=", self.unc_sem_loss is not None,
      "gsu配置=", self.gsu_cfg is not None)

        # 新增=========================================
        if self.geom_sem_loss is not None and self.unc_sem_loss is not None:
            # ===== 进入分支前探针 =====
            print(">>> before GS/US check, step =", self.global_step,
                "geom_sem_loss=", self.geom_sem_loss is not None,
                "unc_sem_loss=", self.unc_sem_loss is not None,
                "apply_after_step=", self.gsu_cfg.get("geom_sem_contrast", {}).get("apply_after_step", 999999),
                "unc_after_step=", self.gsu_cfg.get("unc_sem_couple", {}).get("apply_after_step", 999999),
                "global_step=", self.global_step)
            # ===== 探针结束 =====

            if self.global_step < min(
                self.gsu_cfg.get("geom_sem_contrast", {}).get("apply_after_step", 0),
                self.gsu_cfg.get("unc_sem_couple", {}).get("apply_after_step", 0)
            ):
                return  # 未到生效步，跳过

            if output.get("semantic_latent") is None:
                print(">>> ⚠️  semantic_latent 为 None，跳过 GS/US 计算")
                return

            weight_gs = self.gsu_cfg.get("geom_sem_contrast", {}).get("weight", 0.0)
            weight_us = self.gsu_cfg.get("unc_sem_couple", {}).get("weight", 0.0)
            if weight_gs == 0 and weight_us == 0:
                print(">>> ⚠️  两项权重均为 0，跳过累加")
                return

            print(">>> ✅ GS/US 模块已加载，开始计算loss... step =", self.global_step)

            # 1. 几何–语义对比（潜空间）
            gs_loss = self.geom_sem_loss(output["gaussian_latent"], output["semantic_latent"])

            # 2. 不确定–语义耦合（自监督版）
            sigma_map = output["gaussian_latent"][:, 32:33, :, :]
            sem_feat  = output["semantic_latent"]
            if sigma_map.shape[-2:] != sem_feat.shape[-2:]:
                sigma_map = F.interpolate(sigma_map, size=sem_feat.shape[-2:], mode="nearest")
            us_loss = self.unc_sem_loss(sem_feat, sigma_map, None)

            # 3. 累加 & 日志
            generator_loss += weight_gs * gs_loss + weight_us * us_loss

            print(f">>> 📊 GS_loss = {gs_loss.item():.6f}, US_loss = {us_loss.item():.6f}, step = {self.global_step}")
            print(f">>> ⚖️  GS_weight = {weight_gs}, US_weight = {weight_us}")

            self.log("loss/generator/geom_sem_contrast", gs_loss, on_step=True, on_epoch=True, prog_bar=True)
            self.log("loss/generator/unc_sem_couple", us_loss, on_step=True, on_epoch=True, prog_bar=True)
            print(">>> ✅ GS/US loss计算完成并已记录到日志")
        else:
            print(">>> ❌ GS/US 模块未加载，跳过计算")
        # 新增=========================================
